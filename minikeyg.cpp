...
#include <time.h>
#include <inttypes.h>
#include <sys/time.h> // Ditambahkan untuk gettimeofday
...
volatile unsigned long long total_keys = 0;
...
// Fungsi Thread untuk menampilkan statistik
void *print_status(void *vargp) {
    ...
    while (1) {
        ...
        if (elapsed > 0) {
            double speed = total_keys / elapsed;
            // \r membawa kursor ke awal baris, mencetak ulang statistik
            printf("\r[Checked: %llu keys] [Speed: %.2f keys/s]    ", total_keys, speed);
            fflush(stdout); // Memaksa output langsung tampil
        }
    }
    return NULL;
}
...
int main(int argc, char **argv) {
    ...
    // Membuat thread untuk menampilkan status
    pthread_t status_tid;
    pthread_create(&status_tid, NULL, print_status, NULL);
...
void *process_verified22(void *vargp) {
    ...
    do {
        ...
        // Update counter (4 keys processed)
        __sync_fetch_and_add(&total_keys, 4);

        for (i = 0; i < 4; i++) {
            if (keyvalue[i][0] == 0x00) {
                current_minikey[i][SIZE_VALUE] = '\0';
                // Cetak newline dulu agar tidak nabrak dengan status bar
                printf("\n"); 
                if (FLAG_HEX) {
    ...
    do {
        ...
        for (i = 0; i < 4; i++) {
             if(FLAG_HEX) {
                 unsigned char privkey[32];
                 sha256((unsigned char*)current_minikey[i], SIZE_VALUE, privkey);
                 // Karena mode unverified menghasilkan banyak output, status bar mungkin tidak terlihat jelas
                 // tapi tetap kita coba update
                 printf("\n");
                 bin2hex(privkey, 32);
             } else {
                 printf("\n");
    ...
    do {
        ...
        for (j = 0; j < limit; j++) {
            for (i = 0; i < 4; i++) {
                ...
                // Update counter
                __sync_fetch_and_add(&total_keys, 1); // Disini loopnya per 1, bukan batch 4
